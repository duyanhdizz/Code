...
int n, i, j, a[109][109], check[109][109];
BigInt f[109][109];

BigInt timduong(int x, int y) {
	if(x == n && y == n) return 1;
	BigInt res = 0;
	if(a[x][y] != 0) {
		if(check[x][y + a[x][y]] == 0) {
			BigInt t = timduong(x, y + a[x][y]);
			res += t;
			f[x][y + a[x][y]] = t;
			check[x][y + a[x][y]] = 1;
		} else res += f[x][y + a[x][y]];
		if(check[x + a[x][y]][y] == 0) {
			BigInt t = timduong(x + a[x][y], y);
			check[x + a[x][y]][y] = 1;
			res += t;
			f[x + a[x][y]][y] = t;
		} else res+= f[x + a[x][y]][y];
	} else return 0;
	return res;
}

int32_t main() {
#define TASKNAME "CNTPATH"
