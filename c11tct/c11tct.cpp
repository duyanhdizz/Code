...
const BigInt MOD = 100003;
const BigInt R = 171793;

BigInt f[100003], inv[100003];

BigInt nCk(BigInt n, BigInt k) {
	return ((f[n]) * inv[(f[n - k] * f[k]) % MOD]) % MOD;
}

BigInt calc(BigInt n, BigInt m) {
	BigInt result = 1;
	for (BigInt i = 0; i < 4; i = i + 1) {
		BigInt x = n % MOD;
		n = n / MOD;
		BigInt y = m % MOD;
		m = m / MOD;
		if (x < y) return 0;
		result = fast_mod((result) * nCk(x, y));
	}
	return result;
}
...
#define TASKNAME ""
	...
	f[0] = 1;
	for (BigInt i = 1; i < MOD; i = i + 1) {
		f[i] = fast_mod(uint64_t(f[i - 1]) * i);
	}
	inv[1] = 1;
	for (BigInt i = 2; i < MOD; i = i + 1) {
