...
long long ans2[159][159], ans5[159][159], ans10[159][159];
map<int, int> dem2, dem5, dem10;
...
#define TASKNAME "SUBSET"
	...
	FOR(i, 1, n) {
		FOR(j, 2, min(i, k)) {
			REP(t, j - 1, i) {
				if((min(ans2[i][j], ans5[i][j]) + ans10[i][j]) <= min(ans2[t][j - 1] + dem2[i], ans5[t][j - 1] + dem5[i]) + ans10[t][j - 1] + dem10[i]) {
					ans2[i][j] = ans2[t][j - 1] + dem2[i];
					ans5[i][j] = ans5[t][j - 1] + dem5[i];
					ans10[i][j] = ans10[t][j - 1] + dem10[i];
				}
			}
		}
	}
	long long ans = 0;
	FOR(i, 1, k) ans = max(ans, ans10[n][k] + min(ans2[n][k], ans5[n][k]));
	cout << ans;
