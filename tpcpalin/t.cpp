...
vector<pair<int, int> > pos[N], nex[N][N];
...
//  if (len == 1) return;
  for (int i = 0; i < nex[x][y].size(); ++i) {
    int xx = nex[x][y][i].first;
    int yy = nex[x][y][i].second;
  ...
  if (len > 0) {
    ...
    for (int l = len; l > 1; --l)
      for (int i = 0; i < pos[l].size(); ++i) {
        ...
        if (l < len && nex[x][y].empty()) continue;
        total += nex[x][y].size();
        for (int j = 0; j < pos[l - 1].size(); ++j) {
          ...
          if (xx >= x) break;
          if (xx < x && yy < y) {
            nex[xx][yy].push_back(make_pair(x, y));
          }
        }
      }
    cerr << "total = " << total << '\n';
    for (int i = 0; i < pos[1].size(); ++i) {
      ...
      if (len > 1 && nex[x][y].empty()) continue;
      insert(x, y, 1);
    }
    ans = count(1);
  }
